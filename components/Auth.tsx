...
import { Mail, Lock, User, ChevronRight, Chrome, ArrowRight, UserCircle, RefreshCcw } from 'lucide-react';
...
const Auth: React.FC<AuthProps> = ({ onAuth, lang }) => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-6 animate-in fade-in duration-1000">
      ...
      <div className="w-full max-w-md glass-container rounded-[2.5rem] p-12 space-y-10 shadow-2xl relative border-white/60 bg-white/70">
        ...
        <div className="text-center space-y-3">
          ...
          <p className="text-charcoal/40 text-sm font-bold tracking-tight">
            {mode === 'login' ? 'Continue your rhythm' : 'Your journey to recovery starts here'}
          </p>
        </div>
        ...
        <div className="space-y-5">
          ...
          <div className="relative flex items-center py-4">
            ...
            <span className="flex-shrink mx-4 text-[10px] font-black uppercase tracking-[0.2em] text-charcoal/20">or use email</span>
            <div className="flex-grow border-t border-charcoal/5"></div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {mode === 'signup' && (
              <div className="relative group">
                <User className="absolute left-5 top-1/2 -translate-y-1/2 text-charcoal/20 group-focus-within:text-emerald-500 transition-colors" size={18} />
                <input 
                  type="text" 
                  required
            ...
            <div className="relative group">
              <Mail className="absolute left-5 top-1/2 -translate-y-1/2 text-charcoal/20 group-focus-within:text-emerald-500 transition-colors" size={18} />
              <input 
                type="email" 
                required
            ...
            <div className="relative group">
              <Lock className="absolute left-5 top-1/2 -translate-y-1/2 text-charcoal/20 group-focus-within:text-emerald-500 transition-colors" size={18} />
              <input 
                type="password" 
                required
            ...
            >
              {isLoading ? (
                ...
                <>
                  <span className="text-base">{mode === 'login' ? 'Sign In' : 'Create Account'}</span>
                  <ArrowRight size={20} />
                </>
              )}
            </button>
