# Flow Calendar - AI Agent Router

Ты — системный разработчик Flow Calendar.
Архитектура проекта: гибрид MUI v5 (для стандартных элементов интерфейса) и Tailwind v4 + React Aria (для Expressive UI, таких как блоки времени и холсты).

Перед написанием или изменением кода обязательно прочитай релевантный документ (используй инструмент read_file):
1. UI, дизайн-токены, отступы и базовые правила верстки: читай `docs/ui-rules.md` и `docs/ui-layout-flow.md`.
2. Доступность, работа с фокусом и интерактивные элементы: читай `docs/ui-react-aria-flow.md`.
3. Логика, стейт-менеджмент, контексты и архитектура данных: читай `docs/state-management.md`.
4. Физика (Matter.js), анимации и жесты: читай `docs/gestures-motion.md`.
5. Общая концепция продукта и биомеханика (вкл. left-handed mode): читай `docs/ai-manifesto.md`.

СТРОГИЕ ПРАВИЛА:
- ЗАПРЕЩЕНЫ инлайн-стили (`style={{...}}`).
- ЗАПРЕЩЕНЫ хардкодные HEX-цвета (например, `#10B981`). Используй только CSS-переменные из `tokens.css` или классы Tailwind.
- Glassmorphism (`backdrop-filter: blur`) строго запрещен из-за нагрузки на физический движок.
...
`components/blocks/TimeBlock.tsx` — universal circadian time block card layout.  
`components/blocks/TimeBlockList.tsx` — container for rendering the list of daily time blocks.  
`components/blocks/BubbleBlock.tsx` — interactive physical Matter.js canvas with floating task bubbles inside the Compact Mode.

#### Tasks & Listscomponents
`/tasks/TaskListItem.tsx` — custom interactive task row with checkbox, title, and visual markers supporting deletion with recurrence.  
`components/tasks/WeightBubble.tsx` — reusable static indicator for Weight (Size + Color) and Priority (Shape) operating inside lists.  
`components/TaskManagerPanel.tsx` — bulk task management panel (delete all/completed, display metrics).

#### Modals & Drawers (MUI Standard UI)
`components/modals/TaskSheet.tsx` — bottom sheet drawer (MUI SwipeableDrawer) for manual task entry and updates, fully replacing FocusPoint.  
`components/modals/SettingsModal.tsx` — global application settings panel (wake/rest times, language, and the isLeftHanded biomechanics toggle).  
`components/modals/VoiceConfirmDialog.tsx` — strict MUI dialog window for explicit user confirmation of parsed voice commands.  
`components/modals/AlarmModal.tsx / AlarmPlayingModal.tsx` — circadian alarm interfaces.

#### System Components (Voice, Auth & UI Primitives)
`components/Auth.tsx` — authentication screen handling Guest Mode entry.  
`components/voice/VoiceFeedback.tsx` — visual status indicator and logs for STT voice processing.  
`components/voice/VoiceControlButton.tsx` — dedicated microphone button to toggle voice listening state.  
`components/ui/Button.tsx / components/ui/FAB.tsx` — basic atomic UI primitives built using React Aria. 
`components/blocks/RecoveryBanner.tsx` — status notification banner displayed when the application triggers Recovery Mode.  
...
## Architectural Decision Log (ADL) — June 2026

### ADL-001: Orthogonal Visual Encoding Policy
- **Rule**: Weight and Priority MUST use strictly separate, non-overlapping sensory channels.
- **Implementation**: Weight = Size + Color only. Priority = Geometric Shape only. Priority is forbidden from altering the bubble size or background color. A High-Priority Quick task is a small, mint-colored 12-sided cookie.

### ADL-002: Temporal Stacking & Pointer Events Contract
- **Rule**: Placing `DateNavigator` at `z-10` below `DayView` (`z-20`) must never cause layout clipping or dead click zones.
- **Implementation**: The main `DayView` layout container must use layout margins to prevent physical overlaps with `DateNavigator`. If an overlapping layout is required, `DayView` main container must use `pointer-events-none`, while individual circadian blocks apply `pointer-events-auto`.

### ADL-003: MUI-Tailwind Bridge Isolation
- **Rule**: The dynamic color runtime patch (`attachFlowColorManipulators`) in `mui-theme.ts` is strictly infrastructural code.
- **Implementation**: Business components and style definitions are forbidden from creating hooks or dependencies on this adapter. Style tokens must always be consumed directly via standard `var(--flow-*)` declarations.

### ADL-004: Biomechanical Asymmetry Policy (Left-Handed Mode)
- **Rule**: Left-handed mode is defined as Ergonomic Thumb-Zone Adaptation, not literal geometric mirroring.
- **Implementation**: Elements on the symmetry axis (Central Smart FAB) must remain perfectly centered to prevent device imbalance and accidental edge-presses. Peripheral controllers (navigation arrows, toolbar side icons, header buttons) must mirror their layout positions to enter the active hand's primary interaction arc.